...
`gptcellannotator` connects Seurat workflows to the GPT Cell Annotator engine.
It is CLI-first (offline by default), with optional REST support, validation
guardrails, and plotting helpers.
...
- `GPTCA_BASE_URL` / `GPTCA_API_KEY` only when using the optional REST mode.
...
Only relevant when `offline = FALSE` and calling an external REST service backed
by OpenAI APIs. Keep marker batches small and avoid sharing API keys in saved
objects or public repos.
