...
the GPT Cell Annotator engine. It is CLI-first (offline by default) and can call
an optional REST endpoint when provided with a base URL and API key. This
vignette walks through preparing marker tables, invoking the annotator, applying
validation guardrails, and visualising results on a UMAP embedding.
...
(`GPTCA_CLI_PATH`) take precedence, but explicit configuration is clearer in
...
To target an external REST deployment instead of the CLI, supply a base URL and
API key:

```{r rest-mode, eval=FALSE}
...
`PATH`. Both modes apply the same validation guardrails and return the same
schema.
...
- **Validation disagreements** – inspect `annotations$clusters$validation` for
  ...
  via `annotations$raw`.
- **REST-specific errors (401/429)** – only when `offline = FALSE`. Check API
  credentials and retry settings in `gptca_config()`.
