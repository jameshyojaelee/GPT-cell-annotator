# Annotate Seurat Objects with GPT Cell Annotator

`gptcellannotator` is CLI-first. It shells out to `gca annotate` and
parses the validated JSON report. Live LLM calls are opt-in via
`OPENAI_API_KEY`.
